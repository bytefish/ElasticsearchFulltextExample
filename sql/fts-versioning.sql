...
CREATE OR REPLACE FUNCTION fts.versioning()
...
-- History Tables
CREATE TABLE IF NOT EXISTS fts.document_history (
    LIKE fts.document
);
...
CREATE TABLE IF NOT EXISTS fts.outbox_event_history (
    LIKE fts.outbox_event
);

-- Triggers (Versioning)
CREATE OR REPLACE TRIGGER document_versioning_trigger
BEFORE INSERT OR UPDATE OR DELETE ON fts.document
FOR EACH ROW EXECUTE PROCEDURE fts.versioning(
  'sys_period', 'fts.document_history', true
);

CREATE OR REPLACE TRIGGER document_keyword_versioning_trigger
BEFORE INSERT OR UPDATE OR DELETE ON fts.document_keyword
FOR EACH ROW EXECUTE PROCEDURE fts.versioning(
  'sys_period', 'fts.document_keyword_history', true
);

CREATE OR REPLACE TRIGGER document_suggestion_versioning_trigger
BEFORE INSERT OR UPDATE OR DELETE ON fts.document_suggestion
FOR EACH ROW EXECUTE PROCEDURE fts.versioning(
...
FOR EACH ROW EXECUTE PROCEDURE fts.versioning(
  'sys_period', 'fts.keyword_history', true
);

CREATE OR REPLACE TRIGGER suggestion_versioning_trigger
BEFORE INSERT OR UPDATE OR DELETE ON fts.suggestion
FOR EACH ROW EXECUTE PROCEDURE fts.versioning(
  'sys_period', 'fts.suggestion_history', true
);
...
BEFORE INSERT OR UPDATE OR DELETE ON fts.outbox_event
FOR EACH ROW EXECUTE PROCEDURE fts.versioning(
  'sys_period', 'fts.outbox_event_history', true
);
